...
CUDA_VISIBLE_DEVICES=1  nohup python -u main.py --method $m --root_path ./data/ --n_inner $n --test_bsz $bsz --data PCpower --features M --seq_len 60 --label_len 0 --pred_len $len --des 'Exp' --itr $i --train_epochs 3 --learning_rate 1e-3 --online_learning $online_learning --use_adbfgs >  PCpower$len$m$online_learning.out 2>&1 &
