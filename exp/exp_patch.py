...
class net(nn.Module):
    def __init__(self, configs, device='cuda', max_seq_len: Optional[int] = 1024, d_k: Optional[int] = None,
        ...
        self.model = PatchTST_backbone(c_in=c_in, context_window=context_window, target_window=target_window,
                                       ...
                                       subtract_last=subtract_last, verbose=verbose, channel_cross=self.channel_cross,
                                       **kwargs)
...
class Exp_TS2VecSupervised(Exp_Basic):
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                if self.aug > 0:
                    ...
                    for i in range(self.aug):
                        batch_xa, _ = self.augmenter(batch_x.float().to(self.device),
                                                     torch.ones_like(batch_x).to(self.device))
            ...
            test_loss = 0.
            print(
                "Epoch: {0}, Steps: {1} | Train Loss: {2:.7f} Aug Loss {3:.7f} Vali Loss: {4:.7f} Test Loss: {5:.7f}".format(
